...
namespace ftalertlogicagent
{
	...
	public class Deploy
	{
		...
		static string AWS_AL = "alertlogic.in.ft.com";
		...
		static List<string> DLL_NAMES = new List<string>() { "Heijden.Dns.dll","Newtonsoft.Json.dll","Newtonsoft.Json.FSharp.dll","FSharp.Core.dll","NodaTime.dll"};
		...
		public static void load_dll(string NameSpace, List<string> dlls){
			foreach (string dll in dlls) {
				EmbeddedAssembly.Load (NameSpace + "." + dll,dll);	
			}
		}
		...
		private void Start()
		{
			...
			if (respStr != "") {
				...
				alertlogic_url = last_char_avaib_zone + "." + region +"." + account_id +"."+  AWS_AL;
				alertlogic_get_provkey = account_id +"."+ AWS_AL;
			...
			if (check_prov_key(provkey)) {
				...
				WriteResourceToFile (MSI,AL_INST_MSI);
